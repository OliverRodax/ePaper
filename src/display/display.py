    ...
    def demo(self):
        try:
            ...
            for i, medicine in enumerate(self.data.medicine.today_medicine):
                draw_black.text((i * 200, 0), medicine["name"]+ " Quantity:" + str(medicine["quantity"]),font=self.font_large, fill=0)
            
            for i, plant in enumerate(self.data.plants.plants_water_today):
                draw_black.text((i * 100, 100), plant["name"],self.font_large, fill=0)

            draw_black.text((0, 200),"Weather: " + self.data.weather.weather["main"]+ " Temperature: " + str(self.data.weather.weather["feels_like"]),self.font_large, fill=0)
